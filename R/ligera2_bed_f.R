# uncomment if ligera2_bed.R gets deleted
## #' @useDynLib ligera
...
ligera2_bed_f <- function(
    ...
    # calculate b with this fast code
    ...
    b <- obj$b
    #inbr <- obj$inbr # gets calculated though we don't want it in this version! TODO skip that in cpp
    ...
    while (TRUE) { # start an infinite loop, break inside as needed
        ...
        if ( !is.null( indexes_ind ) ) {
            ...
            Xi <- t( X[ indexes_ind, indexes_loci_chunk, drop = FALSE ] ) # transpose for our usual setup (BEDMatrix)
        } else {
            # keep all individuals (is this faster in that case?)
            Xi <- t( X[ , indexes_loci_chunk, drop = FALSE ] ) # transpose for our usual setup (BEDMatrix)
        }
        ...
        beta[ indexes_loci_chunk ] <- drop( Xi %*% H1[ , 1 ] ) * n_ind_kept / n_ind_no_NA
