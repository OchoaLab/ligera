...
conj_grad_scan_bed_wcpp_rep2 <- function(
                                    file,
                                    m_loci,
    ...
    while ( any( not_converged ) ) {
        ...
        Z[ , not_converged ] <- Z[ , not_converged ] + P * rep(alpha, rep.int(nrow(P), length(alpha)))
        R <- R - KP * rep(alpha, rep.int(nrow(KP), length(alpha)))
        ...
        P <- R + P * rep(Rn1 / Rn, rep.int(nrow(P), length(Rn1 / Rn)))
        Rn <- Rn1
    }
